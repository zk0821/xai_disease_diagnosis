    ...
    def load_pretrained(self):
        if self.type == "densenet121":
            return torch.hub.load(
                "pytorch/vision", "densenet121", weights="IMAGENET1K_V1"
            )
        elif self.type == "densenet169":
            return torch.hub.load(
                "pytorch/vision", "densenet169", weights="IMAGENET1K_V1"
            )
        elif self.type == "densenet201":
            return torch.hub.load(
                "pytorch/vision", "densenet201", weights="IMAGENET1K_V1"
            )
        else:
            raise RuntimeError(
                'Type "{self.type}" is not supported for model DenseNet!'
            )

...
class ChestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Read the image
        img_path = f"{self.path}/images/{self.dataframe['image'].iloc[idx]}.jpeg"
        ...
        if self.policy is not None:
            image_for_mixup = (
                f"{self.path}/images/{self.dataframe['image'].iloc[np.random.randint(0, len(self.dataframe))]}.jpeg"
            )
